...
export default function ClientMessages() {
  ...
  const [tutorProfiles, setTutorProfiles] = useState({}); // id -> name
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        // Fetch tutor names & admin names
        const tutorIds = convs.map(c => c.client_id === u.id ? c.tutor_id : c.client_id);
        const nameMap = {};
        ...
        if (tutorIds.length > 0) {
          ...
          const { data: tutors } = await supabase
            ...
            .in('id', tutorIds);
          tutors?.forEach(t => {
            nameMap[t.id] = t.full_name || 'Unnamed Tutor';
          });
        }
    ...
    <div className="container" style={{ paddingBottom: 'var(--spacing-xl)' }}>
      ...
      {conversations.length === 0 ? (
        <Card style={{ textAlign: 'center', padding: 'var(--spacing-xxl)', color: 'var(--steel)' }}>
          <p>No messages yet. When you contact a tutor or receive a message from the admin, it will appear here.</p>
        </Card>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 'var(--spacing-md)', minHeight: '500px', backgroundColor: 'var(--canvas)', border: '1px solid var(--hairline)', borderRadius: 'var(--rounded-lg)', overflow: 'hidden' }}>
          ...
          <div style={{ borderRight: '1px solid var(--hairline)', overflowY: 'auto' }}>
            {conversations.map(conv => {
              ...
              const name = tutorProfiles[otherPartyId] || 'Support Admin';
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '11px', color: 'var(--steel)' }}>
                        {name === 'Support Admin' ? 'System Notification' : 'Tutor Chat'}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <>
                ...
                <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--hairline)', backgroundColor: 'var(--surface)' }}>
                  <span style={{ fontWeight: 600 }}>
                    {tutorProfiles[selectedConv.client_id === user.id ? selectedConv.tutor_id : selectedConv.client_id] || 'Support Admin'}
                  </span>
                </div>
                ...
                <div style={{ flex: 1, padding: '24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px', backgroundColor: 'var(--surface)', maxHeight: '350px' }}>
                  {messages.map(msg => {
                    ...
                    const senderName = isSelf ? 'You' : (tutorProfiles[msg.sender_id] || 'Support Admin');
                    return (
                      <div
                        key={msg.id}
                        style={{
