  ...
  useEffect(() => {
    ...
    const loadProfile = async (u) => {
      try {
        ...
        console.error('Error loading tutor header profile:', err);
      }
    };
    ...
    const init = async () => {
      try {
        ...
        console.error('Error initializing tutor header:', err);
      }
    };
    init();
    ...
    <nav style={navStyle}>
      <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <Link href="/tutor/dashboard" style={{ marginRight: 'var(--spacing-xl)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            {showDropdown && (
              <div style={{
                ...
                <Link href={`/tutors/${user?.id}`} onClick={() => setShowDropdown(false)} style={{ padding: '10px 16px', fontSize: '13px', color: 'var(--ink)', fontWeight: 500, textDecoration: 'none', cursor: 'pointer', borderBottom: '1px solid var(--hairline-soft)' }}>
                  Profile
                </Link>
                <Link href="/tutor/onboarding" onClick={() => setShowDropdown(false)} style={{ padding: '10px 16px', fontSize: '13px', color: 'var(--ink)', fontWeight: 500, textDecoration: 'none', cursor: 'pointer' }}>
                  Settings
                </Link>
